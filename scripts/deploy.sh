...
echo ''
echo '-----'
echo 'About to create the antidoteDesign_default network if it does'
...
docker network ls | grep antidoteDesign_default || docker network create antidoteDesign_default
...
# docker-compose exec api /scripts/deploy.sh
